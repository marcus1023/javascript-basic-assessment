...
var flashIndex = heroes.indexOf('flash');
var secondHero = heroes[flashIndex];
heroes.splice(flashIndex, 1)
// leave batman in the array but put a copy of him on a variable called thirdHero
var batmansDigit = heroes.indexOf('batman');
var batmansNewName = heroes[batmansDigit];
var thirdHero = batmansNewName;
...
// This is working properly in repl.it... but not jasmine...
...
function doubleTheFun(peram){
	newPeram = peram + peram
	if( peram == Number(peram)){
	  return Number(peram) + Number(peram)
	}
	return newPeram
}
...
function getValueOfProperty(object, keyName){
...
*/
function outside(tempature, humidity, cloudiness){
	if(tempature > 80 && humidity > 40){
		return "I'm all sweat"
	}
	else if(tempature < 40 && cloudiness > 60){
		return "I have icecicles"
	}
	else if(tempature > 80 && humidity < 40 && cloudiness < 20){
		return "I'm literally in the desert"
	}
	else if(tempature > 80 || humidity > 50 || cloudiness > 50){
